...
def show_data():
    ...
    tk.Label(data_window, text="저장 위치", width=30, font=("Arial", 20)).grid(row=0, column=1)
    
    try:
        conn = pymysql.connect(host="127.0.0.1", user="a2b2", passwd="a2b2", db="raspi_db")
        cur = conn.cursor()
        cur.execute("SELECT name, location FROM finger_template")
        for index, (name, location) in enumerate(cur.fetchall()):
            tk.Label(data_window, text=name, width=30, font=("Arial", 16)).grid(row=index+1, column=0)
            tk.Label(data_window, text=location, width=30, font=("Arial", 16)).grid(row=index+1, column=1)
...
def enroll_finger(location, name):
    ...
    if not location_input:
        result_label.config(text="저장 위치를 입력하세요!")
        app.after(3000, clear_message)
        return
    
    elif not name_input:
        result_label.config(text="학번/사번을 입력하세요!")
        app.after(3000, clear_message)
        return
    ...
    try:
        ...
        if existing_record:
            result_label.config(text="이미 사용중인 위치입니다!")
            app.after(3000, clear_message)
        else:
            sql = "INSERT INTO finger_template VALUES (%s, %s, %s)"
            cur.execute(sql, (characteristics, name, location))
            conn.commit()
            result_label.config(text="지문 등록 성공!")
            app.after(3000, clear_message)
    except Exception as e:
...
def search_fingerprint():
    try:
        ...
        for row in cur.fetchall():
            ...
            if score > best_score:
                best_score = score
                identified_user = row[1]
                location = row[2];

        if identified_user:
            result_label.config(text=f"저장 위치 {location}번에서 {identified_user} 감지됨")
        else:
        ...
        print('Operation failed!')
        print('Exception message: ' + str(e))
        result_label.config(text="지문 검색에 실패함")
    finally:
        conn.close()
        app.after(3000, clear_message)
...
def delete_fingerprint():
    ...
    if not location_input:
        result_label.config(text="삭제할 위치를 선택하세요!")
        app.after(3000, clear_message)
        return
    ...
    try:
        ...
        result_label.config(text="잘못된 위치 값입니다. 숫자를 입력하세요.")
        app.after(3000, clear_message)
        return

    result_label.config(text="삭제중...")
    ...
    try:
        ...
        if affected_rows > 0:
            result_label.config(text=f"{location}에 위치한 지문 정보 삭제 완료!")
        else:
            result_label.config(text="삭제할 지문 정보가 없습니다.")
...
def clock_in():
    try:
        ...
        if identified_user:
            ...
            server_response = send_to_server("출근", identified_user)
...
# 배경 이미지 로드
bg_image = tk.PhotoImage(file="/home/a2b2-2/Pictures/konkuk_logo.png")
...
# 이름과 지문 데이터를 저장할 딕셔너리 초기화
fingerprint_data = {}

# 지문 등록 UI 구성
name_label = tk.Label(frame, text="학번:", font=("Arial", 24))
...
# 지문 등록 버튼 생성
enroll_button = tk.Button(button_frame, text="지문 등록", command=lambda: enroll_finger(location_entry.get(), name_entry.get()), font=("Arial", 24))
...
# 결과 표시 레이블
result_label = tk.Label(frame, text="", font=("Arial", 24))
